...
class InscripcionRepository:
    ...
    def add(self, inscripcion):
        data = InscripcionData(
            ...
            administradores='' if inscripcion.administradores is None
            else ','.join(inscripcion.administradores))
    ...
    def find_all_by_admin(self, admin):
        ...
        return list(
            filter(
                lambda i: admin in i.administradores,
                inscripciones))

    def get_total_amount(self, inscripcion_id):
        query = 'select sum(monto) as total_amount ' \
                'from participantes where inscripcion_id = :id'
        if inscripcion_id is None:
            raise Exception('Inscripcion id is not valid')
        response = self.session.execute(query, {
            'id': str(inscripcion_id)}).fetchone().total_amount
        if response is None:
            return Decimal('0.00')
        else:
            return response
...
class ParticipanteRepository:
    ...
    def update(self, participante):
        data = ParticipanteData.query.filter_by(
            id=str(participante.id)).first()
    ...
    def find_all(self, inscripcion_id):
        data_list = ParticipanteData.query.filter_by(
            inscripcion_id=str(inscripcion_id)).all()
        res = map(
            lambda data: Participante(
                id=uuid.UUID(
                    data.id),
                nombres_completos=data.nombres_completos,
                sexo=data.sexo,
                telefono_contacto=data.telefono_contacto,
                monto=Decimal('0.00') if data.monto is None else Decimal(
                    data.monto),
                fecha_inscripcion=data.fecha_inscripcion,
                numero_deposito=data.numero_deposito),
            data_list)
